...
object NuBracket {
  ...
  def main(args: Array[String]) {
    ...
    val l = BracketLeaf(PingpongMatch("123",Some(Player("1","Koen","Van Loock", Ranks.D0)), Some(Player("3","Luk","Geraets", Ranks.D6)),"1",3,true,21,2,2,0,List(PingpongGame(21,15,1), PingpongGame(21,13,2))))
    ...
    def createSets(siteMatch: PingpongMatch): PingpongMatch = {
      val createdGames  = (1 to 2*siteMatch.numberOfSetsToWin -1).map(gameNr => PingpongGame(0,0,gameNr)).toList
      siteMatch.copy(games = createdGames)
    }


    val matchList = List(
      PingpongMatch("123",Some(Player("1","Koen","Van Loock", Ranks.D0)), Some(Player("3","Luk","Geraets", Ranks.D6)),"1",3,true,21,2,2,0,List(PingpongGame(21,15,1), PingpongGame(21,13,2))),
      PingpongMatch("124",Some(Player("1","Koen","Van Loock", Ranks.D0)), Some(Player("3","Luk","Geraets", Ranks.D6)),"1",3,true,21,2,2,0,List(PingpongGame(21,15,1), PingpongGame(21,13,2))),
